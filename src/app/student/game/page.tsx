...
export default function GamePage() {
  ...
  const [kanaMode, setKanaMode] = useState<'hiragana' | 'katakana'>('hiragana') // 日文鍵盤模式
  ...
  const handleStartGame = () => {
    ...
    // 隨機選擇一個單字
    const availableWords = selectedVocab.words.filter((w: Word) => w.word && w.word.trim().length > 0)
    ...
    const isJapanese = isJapaneseLanguage(selectedLangUse)
    // 日文保持原樣，英文轉小寫
  ...
  const isJapanese = isJapaneseLanguage(selectedLangUse)
    ...
    if (guess === targetWordText) {
      ...
      finishGame(true, attemptsUsed)
    } else if (newGuesses.length >= maxAttempts) {
      setGameLost(true)
      finishGame(false, maxAttempts)
    }
  }

  const finishGame = async (won: boolean, attempts: number) => {
    setStage('result')
    
    // 計算點數（獲勝才有點數）
    if (won) {
      const accuracy = 100
      const totalTime = attempts * 10000 // 估算時間
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (stage === 'playing' && !gameWon && !gameLost) {
        const isJapanese = isJapaneseLanguage(selectedLangUse)
  ...
  }, [stage, currentGuess, targetWordText, gameWon, gameLost, selectedLangUse])
          ...
          <Paper sx={{ p: 4 }}>
              ...
              <Card sx={{ mb: 4, bgcolor: 'primary.light', color: 'primary.contrastText' }}>
                <CardContent>
                  ...
                  <Typography variant="body2" sx={{ mt: 2, opacity: 0.9 }}>
                    單字長度: {targetWordText.length} 個{isJapaneseLanguage(selectedLangUse) ? '字符' : '字母'}
                  </Typography>
                </CardContent>
              </Card>
            )}
            ...
            {(gameWon || gameLost) && (
              <Box sx={{ display: 'flex', gap: 2, justifyContent: 'center' }}>
                ...
                <Button variant="contained" onClick={() => setStage('result')}>
                  查看結果
                </Button>
              </Box>
            )}
