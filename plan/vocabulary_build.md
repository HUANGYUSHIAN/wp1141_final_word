...
本文檔定義了使用 LLM（OpenAI GPT-4o-mini）查詢單字時的所有規範和規則。
...
所有 `langUse` 和 `langExp` 參數必須是上述四種語言之一。

## 2. 單字要求

- **生成策略**：根據輸入的單字或資訊，請求用langUse生成單字，並提供同義字、反義字與相關字
- **一字多義**：如果該單字一字多意，每個意思都要造句，Word.Word會相同，但是Word.Explanation & Word.Sentence會不同
...
#### 句子位置規則
- **前兩次嘗試**：盡量避免整本單字本中，`<單字<` 幾乎出現在句子一開頭或最尾端
  - 要適量放在句子中間，可配合時態等做變化
  - ❌ 避免範例（盡可能避免）：
    - 開頭：`<天氣<如何`、`<天気<はどうですか`、`<Apple< is one kind of fruit`
    - 結尾：`花子は来週日本へ帰ると<言う<。`、`我喜歡吃<蘋果<。`
  - ✅ 正確範例：
    - 日文：`<運営<が円滑に行われています。`（單字在句子中間）
    - 韓文：`손에는<가방<을들고있었어요。`、`라면을 끓여<먹다<。`（單字在句子中間）
- **第三次嘗試**（單字未滿 25 個時）：此規則放寬，`<單字<` 可以出現在開頭、中間或結尾
...
```json
{
  ...
  "explanation": "管理、經營",
  "partOfSpeech": "動詞",
  "sentence": "He <manages< a restaurant."
}
```