...
**⚠️ LLM 使用額度說明**：
- 系統預設每日 LLM 使用額度為 **0.001 美金**
- 當用戶的每日 LLM 使用額度超過限制時，所有 LLM 相關功能（AI 生成單字本、文法家教等）會被鎖定
- 管理員可以透過 `/admin/setting` 調整每日 LLM 額度設定
- 建議在開發或測試時，可以適當提高額度以避免功能被鎖定
...
若不想透過 Google OAuth 登入，可以使用測試登入功能。使用方式：

1. 前往登入頁面：http://localhost:3000/login
2. 選擇「測試登入」
3. 輸入以下任一測試帳號的 User ID 進行登入

**預設測試帳號**：

| User ID | 名稱 | Email | 角色 |
|---------|------|-------|------|
| `cMbCvXr3fsEhNJLQF8dJpXfIAjnG3E` | test_student1 | test_student1@gmail.com | Student |
| `Lkc9IO0ez1b87VtNQVWvVUGyQqUKIE` | yun | yunn030123@gmail.com | Supplier |
| `SsDyoyPfwyCLXXhn1KZBvmHhIRfKrD` | Admin User | admin1763134902258@example.com | Admin |
...
> **注意**：執行 `npm run db:seed-test-data` 也會建立新的測試帳號，並顯示對應的 User ID。
...
**使用方式**：
- 選擇單字本（單字數需大於 20，單字數不足的單字本「+」按鈕會被禁用）
- 點擊「+」圖示開啟設定對話框
- 設定題目數量（最多為單字本總數）
...
**注意事項**：
- 單字本單字數必須大於 20 才能進行測驗
- 單字數不足 20 的單字本在列表中會顯示禁用的「+」按鈕
...
**注意事項**：
- 所有遊戲都要求單字本單字數必須大於 20
- 單字數不足 20 的單字本不會出現在下拉選單中
- 如果嘗試使用單字數不足的單字本，系統會彈出提示對話框
...
**使用方式**：
- **LLM 使用額度**：
  - 設定每日 LLM 額度（美金），預設為 0.001 美金
  - 查看剩餘額度或總消耗額度
  - **重要**：當用戶的每日 LLM 使用額度超過限制時，所有 LLM 相關功能（AI 生成單字本、文法家教等）會被鎖定
  - 管理員可以在此調整每日額度，建議在開發或測試時適當提高額度
...
黃宇賢(YS)心得：
  我非常感謝黃鐘揚老師、助教以及所有課程相關人員。在修這門課之前，我從未想過自己能夠做出如此有趣且實用的網路服務，更沒想到能遇到像 Cursor 這樣強大的開發工具，協助我完成一個如此完整的成品。這個作品不僅讓我想實際拿來使用，也讓我產生分享給親朋好友，甚至進一步發展為創業專案的想法。
  三年前，我曾在交大電機系修過另一位黃老師開設的網頁程式設計課程。當時是以 C 語言從零開始手刻所有功能，最終只能在 local 端執行。三年後的今天，透過這堂課，我學會了現代網路服務的基本架構與常用套件，並能夠結合 Cursor 打造出真正符合自己想像的網頁服務，這樣的成長讓我由衷感到感恩。
  回想三年前修英文課時，我曾因需要快速檢驗背單字的學習成果，自行撰寫了一個 Python GUI 小程式。當時英文老師便曾詢問，是否有可能將這樣的工具做成大家都能使用的平台，甚至串聯字典，讓學生能快速建立屬於自己的單字本，並透過例句理解單字的實際用法。三年後，透過這門網路服務課程，我終於清楚知道該如何實現這些構想，甚至能做得更完整、更理想，於是便誕生了這次的 Final Project。
  最後，我也非常感謝孫寅鈞同學願意與我一同圓夢，合作開發這個背單字平台。我們幾乎將本學期超過八成的技術內容整合進這個專案中，成果也遠遠超出了最初規劃的服務範圍。這段合作與學習的過程，對我而言是非常珍貴且難忘的經驗。
